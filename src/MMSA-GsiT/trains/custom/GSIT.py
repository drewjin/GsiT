...
class GsiT():
    def __init__(self, args):
        ...
        # self.bert_tokenizer = BertTokenizer.from_pretrained(args.weight_dir)
    ...
    def param_flops(self, model, cat_seq):
        macs, params = profile(model, inputs=cat_seq)
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        bert_params = list(model.Model.bert_model.named_parameters())
        ...
        model_params_other = [p for n, p in list(model.Model.named_parameters()) if 'bert_model' not in n and \
        ...
        while True: 
            ...
            # train
            y_pred = {'M': [], 'T': [], 'A': [], 'V': [], 'final_out': []}
            y_true = {'M': [], 'T': [], 'A': [], 'V': [], 'final_out': []}
            losses = []
            model.train()
            train_loss = 0.0
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # vision_rgb = batch_data['video_rgb'].to(self.args.device)
                    # audio_wav = batch_data['audio_wav'].to(self.args.device)
                    vision_feat = batch_data['vision'].to(self.args.device)
                    audio_feat = batch_data['audio'].to(self.args.device)
                    text = batch_data['text'].to(self.args.device)
                    labels_m = batch_data['labels']['M'].to(self.args.device).view(-1, 1)
                    indexes = batch_data['index'].view(-1)
                    cur_id = batch_data['id']
                    ids.extend(cur_id)

                    if not self.args.need_data_aligned:
                        # audio_lengths = batch_data['audio_wav_lens']
                        # vision_lengths = batch_data['video_rgb_lens']
                    ...
                    outputs,_,_ = model(
                        ...
                        (audio_feat, audio_lengths), 
                        (vision_feat, vision_lengths), 
                        plt=False
                    )
                    ...
                    # print(f"Total parameters: {self.count_params(model) / 1e6} M")

                    # self.param_flops(
                    #     model=model, 
                    #     cat_seq=((input_ids, input_mask, segment_ids), 
                    #     (audio_feat, audio_lengths), 
                    #     (vision_feat, vision_lengths), 
                    #     False)
                    # )

                    # y_pred['final_out'].append(outputs['final_out'].cpu())
                    # y_true['final_out'].append(labels_m.cpu())
                    # store results
                    ...
                    # compute loss
                    loss = 0.0
                    # loss += self.criterion(outputs['final_out'], labels_m)
                    for m in self.args.tasks:
                    ...
                    # update features # TODO down to GsiT
            ...
            # pred, true = torch.cat(y_pred['final_out']), torch.cat(y_true['final_out'])
            # train_results = self.metrics(pred, true)
            # logger.info('FINAL_OUT: >> ' + dict_to_str(train_results))
            # validation
    ...
    def do_test(self, model, dataloader, mode="VAL", return_sample_results=False):
        ...
        with torch.no_grad():
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    loss_1 = self.weighted_loss(outputs['M'], labels_m)
                    # loss_2 = self.criterion(outputs['final_out'], labels_m.view(-1, 1))
        ...
        logger.info(mode+"-(%s)" % self.args.model_name + " >> loss_1: %.4f " % eval_loss_1)
        pred, true = torch.cat(y_pred['M']), torch.cat(y_true['M'])
        eval_results = self.metrics(pred, true)
        logger.info('M: >> ' + dict_to_str(eval_results))
        # pred, true = torch.cat(y_pred['final_out']), torch.cat(y_true['final_out'])
        # eval_results = self.metrics(pred, true)
        # logger.info('FINAL_OUT: >> ' + dict_to_str(eval_results))
        eval_results['Loss'] = round(eval_loss_1, 4)
