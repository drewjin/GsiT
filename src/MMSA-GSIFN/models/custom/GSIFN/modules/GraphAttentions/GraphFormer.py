...
class GraphFormerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None, mask=None, mask_fixer=None, plot_map=False):
        """
        Args:
        ...
        for layer in self.layers:
            if x_in_k is not None and x_in_v is not None:
                x, weights = layer(x, x_k, x_v, mask, mask_fixer, plot_map=plot_map)
            else:
                x, weights = layer(x, mask=mask, mask_fixer=mask_fixer, plot_map=plot_map)
...
class GraphFormerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, mask=None, mask_fixer=None, plot_map=False):
        """
        Args:
