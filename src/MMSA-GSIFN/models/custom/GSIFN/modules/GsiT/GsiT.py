...
class GsiT(nn.Module):
    def __init__(self, args):
        ...
        if not cfg.bidirectional:
            ...
            if not self.bidi:
                self.cross_mask_forward = None
                self.cross_mask_backward = None
                self.self_mask = None
            else:
                self.cross_mask_bidi = None
                self.self_mask = None
    
    def forward(self, cat_seq, split, plot_map):
        if not self.bidi:
            if (self.cross_mask_forward is None and 
                self.cross_mask_backward is None and 
                self.self_mask is None):
                self.cross_mask_forward = cross_mask_forward = self.build_adj_masked_matrix(
                    split, mode='cross', direction='forward'
                ).to(cat_seq.device)
                self.cross_mask_backward = cross_mask_backward = self.build_adj_masked_matrix(
                    split, mode='cross', direction='backward'
                ).to(cat_seq.device)
                self.self_mask = self_mask = self.build_adj_masked_matrix(
                    split, mode='self'
                ).to(cat_seq.device)
            else:
                cross_mask_forward = self.cross_mask_forward
                cross_mask_backward = self.cross_mask_backward
            ...
            if (self.cross_mask_bidi is None and
                self.self_mask is None):
                self.cross_mask_bidi = cross_mask_bidi = self.build_adj_masked_matrix(
                ...
                cross_mask_bidi = self.cross_mask_bidi
                self_mask = self.self_mask
